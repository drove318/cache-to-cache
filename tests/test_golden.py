...
class TestMainResults:
    ...
    def test_the_gains_are_the_published_ones(self):
        t = TABLES["table04"]
        qwen, llama, big = (
            t["sharers"][name] for name in ("Qwen2.5-0.5B-Instruct", "Llama3.2-1B", "Qwen3-4B-Base")
        )
...
class TestTheRunner:
    ...
    def test_a_cheat_oracle_scores_a_hundred_percent(self, tmp_path):
        ...
        rows = [
            {"question": f"q{n}", "choices": ["one", "two", "gamma"], "answer": 2} for n in range(5)
        ]
        with open(fixture, "w", encoding="utf-8") as fh:
            for row in rows:
                fh.write(json.dumps(row) + "\n")
        result = run("arc-c", ask=lambda prompt: "gamma", mode="oracle", fixtures_dir=str(tmp_path))
    ...
    def test_the_ignorant_oracle_scores_a_zero(self, tmp_path):
        ...
        with open(fixture, "w", encoding="utf-8") as fh:
            for n in range(3):
                fh.write(
                    json.dumps({"question": f"q{n}", "choices": ["x", "y", "z"], "answer": 2})
                    + "\n"
                )
        result = run("arc-c", ask=lambda prompt: "nothing at all here", fixtures_dir=str(tmp_path))

...
class TestEffectiveRank:
    ...
    def test_buffers_are_accepted_from_any_sequence(self):
        ...
        assert effective_rank(plain) == pytest.approx(
            effective_rank(torch.tensor(plain)), rel=1e-6)
...
class TestRankReport:
    ...
    def test_the_comparison_is_made(self):
        ...
        assert report.key["delta"] == pytest.approx(
            report.key["after"] - report.key["before"])

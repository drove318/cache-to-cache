...
INITIALISE = {
    ...
    "id": 1,
    "method": "initialize",
    "params": {"protocolVersion": "2025-06-18", "clientInfo": {"name": "pytest", "version": "1"}},
}
...
class TestTheProtocol:
    ...
    def test_tools_are_listed(self):
        ...
        server, _, writer = _server()
        replies = _ask_server(server, writer, {"jsonrpc": "2.0", "id": 2, "method": "tools/list"})
    ...
    def test_ping_responds_pong(self):
        server, _, writer = _server()
        replies = _ask_server(server, writer, {"jsonrpc": "2.0", "id": 5, "method": "ping"})
    ...
    def test_unknown_method_is_method_not_found(self):
        ...
        server, _, writer = _server()
        replies = _ask_server(server, writer, {"jsonrpc": "2.0", "id": 6, "method": "tools/spin"})
...
class TestAgentCard:
    ...
    def test_the_card_declares_the_agent(self):
        ...
        assert any(
            "c2c" in (s.get("id", "") + s.get("name", "")).lower()
            or "cache" in s.get("id", "").lower()
            for s in skills
        )  # the skills, C2C
...
class TestTheHTTPTransport:
    ...
    def port(self):
        ...
        saved = (
            MCPRequestHandler.server_core,
            MCPRequestHandler.api_key,
        ...
        (MCPRequestHandler.server_core, MCPRequestHandler.api_key, MCPRequestHandler.verbose) = (
            saved
        )

    def test_the_health_of_the_second_transport(self, port):
        with urllib.request.urlopen(f"http://127.0.0.1:{port}/healthz", timeout=20) as response:
    ...
    def test_an_unknown_method_is_an_error_object(self, port):
        _status, reply = _http_rpc(port, {"jsonrpc": "2.0", "id": 3, "method": "tools/frobnicate"})
    ...
    def test_the_weights_path_must_mind_the_zoo_root(self):
        ...
        replies = _ask_server(
            ...
            writer,
            {
                ...
                "method": "tools/call",
                "params": {
                    "name": "c2c_register_pair",
                    "arguments": {"receiver": "r", "sharer": "s", "weights_path": "/etc/passwd.pt"},
                },
            },
        )
        error = replies[0]["error"]

...
DISTRIBUTION = ("reference", "hf", "vllm", "sglang", "trtllm", "llamacpp", "tgi", "ollama", "mlx")
...
class TestTheBus:
    ...
    def test_entry_points_extend_the_registry(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            "entry_points",
            lambda **kw: (
                importlib.metadata.EntryPoints([ep])
        ...
        assert (
            engines.lookup("external-engine") == "c2c.integrations.reference:ReferenceAdapter"
        )  # loaded, as declared

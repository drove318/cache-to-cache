...
SHARERS = [
    LayerGeometry(layers=3, hidden_size=12, num_heads=3, name=f"sharer-{n}") for n in range(3)
]
...
class TestUnifiedLatentSpace:
    ...
    def test_the_budget_grows_linearly_in_the_layers(self):
        ...
        for n in (2, 3, 4):
            space = self.make()
            totals.append(sum(p.numel() for p in space.fusers[:n].parameters()) + p_num_gate(space))
        first = [b - a for a, b in zip(totals, totals[1:])]
...
class TestTheShelf:
    ...
    def test_publish_writes_the_manifest_and_the_weights(self):
        manifest = self.client.publish(
            fuser=self._train_a_fuser(), sharer="sharer-0", receiver="receiver-mini"
        )
        pid = manifest["id"]
    ...
    def test_an_unsealed_manifest_is_refused(self):
        ...
        manifest = self.client.publish(
            fuser=self._train_a_fuser(), sharer="sharer-9", receiver="receiver-mini"
        )
        ...
        with pytest.raises(ValueError, match="no sha256 seal"):
            self.client.load_fuser(
                sharer="sharer-9", receiver="receiver-mini", fuser_builder=lambda: None
            )
    ...
    def test_fetch_round_trips_the_fuser(self):
        ...
        restored = self.client.load_fuser(
            sharer="sharer-0",
            receiver="receiver-mini",
            fuser_builder=lambda: Fuser(RECEIVER, SHARERS[0], [0, 1, 2]),
        )

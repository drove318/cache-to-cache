...
def make_fuser(**kw):
    ...
    gc = kw.pop("gate_config", None) or GateConfig(**{k: kw.pop(k) for k in list(kw) if k in gkeys})
    ...
    return Fuser(RECEIVER, SHARER, MAPPING, fuser_config=fc, gate_config=gc, blend_config=bc)
...
class TestFusionEquation:
    ...
    def test_the_gate_is_soft_and_differentiable_at_training(self):
        ...
        assert any(
            p.grad is not None and p.grad.abs().sum() > 0 for p in fuser.gate.parameters()
        )  # differentiable, despite
...
class TestGradientFlow:
    ...
    def test_gradients_flow_through_the_straight_through_estimator(self):
        ...
        fused = fuser(CACHE_R(), CACHE_S())
        loss = (
            fused.as_matrix().pow(2).mean()
            if hasattr(fused, "as_matrix")
            else torch.cat([slc.key.flatten() for slc in fused]).pow(2).mean()
        )
        loss.backward()

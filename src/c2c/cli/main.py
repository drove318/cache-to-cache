...
def _add_model_arguments(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "--receiver", required=True, metavar="ID", help="receiver model id (the one that speaks)"
    )
    parser.add_argument(
        "--sharer", required=True, metavar="ID", help="sharer model id (the one that shares)"
    )
...
def cmd_doctor(args: argparse.Namespace) -> int:
    ...
    if args.report:
        ...
        rows = [
            {"check": c.name, "status": c.status, "detail": c.detail, "hint": c.hint}
            for c in checks
        ]
...
def cmd_fuse(args: argparse.Namespace) -> int:
    ...
    mapper = terminal_mapping if cfg.align.layers == "terminal" else depth_normalized_mapping
    ...
    fuser = Fuser(
        ...
        blend_config=BlendConfig(
            fraction=args.fraction, direction=args.direction or cfg.blend.direction
        ),
    )
    fuser.eval()
    import torch
    ...
    if agreement < 0.80:
        print(
            error_hint(
                f"selection strategies agree on only {agreement:0.1%}",
...
def cmd_train(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(
            error_hint(
                f"cannot build the engines: {exc}",
                hint="try `--engine reference` (always available)",
            ),
            file=sys.stderr,
        )
        return 1
    ...
    trainer = Trainer(
        ...
        adapter_s,
        adapter_r,
        receiver_tokenizer=adapter_r,
        sharer_tokenizer=adapter_s,
        recipe=recipe,
    )
    ...
    if not dataset:
        print(
            error_hint(
                "the dataset stream is empty",
                hint="check the JSON-L file: every line needs a context and a response",
            ),
            file=sys.stderr,
        )
        return 1
...
def cmd_zoo(args: argparse.Namespace) -> int:
    ...
    if action == "publish":
        if not args.checkpoint or not os.path.isfile(args.checkpoint):
            print(
                error_hint(
                    "publish needs --checkpoint PATH (a .pt file)",
                    hint="train first: c2c train ... --out model.pt",
                ),
                file=sys.stderr,
            )
            return 1
        ...
        manifest = client.publish(
            sharer=args.sharer, receiver=args.receiver, fuser=_StateCarrier(state)
        )
    ...
    print(
        error_hint(f"unknown zoo action {action!r}", hint="actions: list, publish, get, remove"),
        file=sys.stderr,
    )
    return 1
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("-s", "--strict", action="store_true", help="exit non-zero when any check fails")
    p.add_argument(
        "--report",
        action="store_true",
    ...
    _add_seed_device(p)
    p.add_argument(
        "-d", "--dataset", required=True, metavar="FILE", help="JSON-Lines file of training samples"
    )
    ...
    p.add_argument("extras", nargs=argparse.REMAINDER, default=[], help="see c2c-serve --help")
    p.set_defaults(func=cmd_serve)

    p = sub.add_parser("eval", help="run the golden regression against the paper")
    p.add_argument("target", nargs="*", default=None)
    p.add_argument("--table", type=int, action="append", help="run only Table N (repeatable)")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            error_hint(
                "the ML backend is absent: this command fits fusers, and needs torch",

...
class vLLMAdapter(EngineAdapter):  # noqa: N801 — the engines brand spelling
    ...
    engine_name = "vLLM"
    required_extra = "vllm"
    DEGRADATION = ("no connector API in this build: prefill-only capture, "
    ...
    def __init__(self, model_id: str, **options):
        ...
        if not available():
            raise AdapterNotSupported("the 'vllm' adapter needs the vllm package",
    ...
    def _build_spec(self) -> ModelSpec:
        ...
        cfg = engine.model_config.hf_config if hasattr(engine, "model_config") else \
        ...
        kind = (AttentionKind.MHA if kv_heads == heads else
               AttentionKind.MQA if kv_heads == 1 else AttentionKind.GQA)
        return ModelSpec(
            id=self.model_id,
            geometry=LayerGeometry(layers=layers, hidden_size=hidden, num_heads=heads,
                                 num_key_value_heads=kv_heads, attention=kind,
                                 name=self.model_id),
    ...
    def capture(self, prompt_tokens):
        ...
        engine = self._ensure_engine()
        blocks = engine.collect_prefix_cache(list(prompt_tokens),

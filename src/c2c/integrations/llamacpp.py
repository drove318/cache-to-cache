...
class LlamaCppAdapter(EngineAdapter):
    ...
    DEGRADATION = ("the bindings keep the cache behind opaque handles: "
    ...
    def __init__(self, model_id: str, **options):
        ...
        if not available():
            raise AdapterNotSupported("the 'llama-cpp' adapter needs llama-cpp-python",
    ...
    def _ensure(self):
        if self._ctx is None:
            model = self._lc.Model.from_file(self.model_id,
                                          vocab=self.options.get("vocab"),
    ...
    def _build_spec(self) -> ModelSpec:
        ...
        return ModelSpec(id=self.model_id,
                        geometry=LayerGeometry(layers=n_layer, hidden_size=max(n_embd, 1),
                                            num_heads=max(n_head, 1), name=self.model_id),
                        family="llama.cpp",
    ...
    def generate(self, prompt_tokens, *, max_new_tokens: int = 64, temperature: float = 0.0,
        ...
        sampler = ctx.get_default_sampler(
            mapping=self._lc.DEFAULT_SAMPLER_MAPPING,
            temperature=float(temperature or 0.0))
        return sampler.complete(ctx, prompt=text,

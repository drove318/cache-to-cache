...
def build_agent_card(config: ServeConfig | None = None, hub=None) -> dict:
    ...
    return {
        ...
        "name": "c2c-serve",
        "description": (
            "Cache-to-Cache middleware: direct semantic communication "
        ...
        "supported_interfaces": [
            {
                "url": base + ROUTE_CHAT_COMPLETIONS,
                "protocol_binding": "REST",
                "protocol_version": "1.0",
            },
            {
                "url": base + ROUTE_COMPLETIONS,
                "protocol_binding": "REST",
                "protocol_version": "1.0",
            },
        ],
        "skills": [
            {
                ...
                "description": (
                    "Fuse the KV-cache of a Sharer model into a Receiver "
        ...
        "security_schemes": {
            "bearer": {
                ...
                "scheme": "bearer",
                "description": (
                    "required when the server was started with "
...
class TaskStore:
    ...
    def submit(self, text: str, *, context_id: str | None = None, owner: str = "") -> dict:
    ...
    def set_state(self, task_id: str, state: str, *, artifacts: list | None = None) -> dict | None:
...
class A2AHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802
        path = self.path.split("?", 1)[0]
        if path != TASK_ROUTE:
            self._json({"error": {"message": f"no such route: {path!r}"}}, HTTPStatus.NOT_FOUND)
            return
        ...
        try:
            ...
            self._rpc_error(
                req_id, -32603, f"internal error: {exc.__class__.__name__} (see the server log)"
            )
    ...
    def _dispatch(self, method: str, req_id, params: dict, principal: str) -> None:
        ...
        if method == "tasks/list":
            ...
            rows = [
                t
                for t in store.listed(context_id=str(context_id))
                if t.get("owner", "") == principal
            ]
            self._json(
                {"jsonrpc": "2.0", "id": req_id, "result": {"tasks": rows, "next_page_token": ""}}
            )
            return
    ...
    def _send(self, pipeline, store: TaskStore, req_id, params: dict, principal: str) -> None:
        ...
        task = store.submit(
            text, owner=principal, context_id=str(context_id) if context_id else None
        )
            ...
            self._json(
                {
                    ...
                    "result": {
                        **(failed or task),
                        "error": f"{exc.__class__.__name__}: the task failed (see the server log)",
                    },
                }
            )
            return
        ...
        artifact = [
            {
                "artifact_id": f"artifact-{task['id']}-0",
                "name": "answer",
...
def serve_bridge(
    config: ServeConfig | None = None, *, hub=None, public_url: str | None = None
) -> None:
...
def build_parser(prog: str = "c2c-a2a") -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--public-url", default=None, help="the url the card advertises (default: the bind address)"
    )
    parser.add_argument(
        "--privacy",
        action="store_true",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    cfg = ServeConfig(**{**vars(serve_base), **{k: v for k, v in updates.items() if v is not None}})
    if not cfg.api_key and not is_loopback(cfg.host):
        from ..utils.console import error_hint

        print(
            error_hint(
                "the bridge, off the loopback, must have a key: --api-key KEY, "
